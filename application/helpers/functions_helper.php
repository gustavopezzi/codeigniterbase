...
if (!function_exists('getMonthBR')){
	function getMonthBR($date, $abbr=false, $lc=false)
	{
		$ret = '';
		switch($date) {
			case 1:
...
if (!function_exists('format_data_time')){
	function format_data_time($date,$just_hour=false){
...
if (!function_exists('DolartoReal'))
{
   function DolartoReal($valor, $rs=true){
      if ($rs)
         return $valor == 0 ? "R$ 0,00" : "R$ ".number_format($valor, 2, ',', '.');
      else
         return $valor == 0 ? "0,00" : number_format($valor, 2, ',', '.');
   }
}
